...
const generateTestCases = flow(function* (
  self: Store,
...
It is also checking division by zero when it is not the obligation of this test scenario to check that.`,
        ...
        {
          role: "user",
          content: `You should only generate test cases for this particular test scenario for now and the generated test cases should not belong to other test scenarios: ${testScenario?.content}`,
        },
      ],
    })
  );
  ...
  if (testCases == null) {
    throw new Error("No response generated by ai for test cases!");
  }

  testScenario?.setTestCases(prepareContent(testCases));
  self.eventTarget.emit("iterationUpdate", Iteration.testCases);
}) as (self_: unknown, testScenario?: TestScenario) => Promise<void>;
