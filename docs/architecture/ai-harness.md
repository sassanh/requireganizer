...
3. For the commanded stage, exactly one result tool (`submit_*`) is offered alongside the reads. Its arguments carry the complete proposal. Free-form turns keep the most recent command's result tool until a newer command supersedes it, so an interrupted submission stays retryable; re-issuing a command whose turn ended in an error replaces that dead attempt instead of duplicating it.
