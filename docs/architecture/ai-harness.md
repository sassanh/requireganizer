...
3. Every turn carries the **full submission toolset**: one result tool (`submit_*`) per stage whose prerequisites currently hold, plus `activate_stage_result_tool` as an escape hatch that validates prerequisites and reports what is missing. Construction is the gate — stages with unmet prerequisites stay absent until the workflow unlocks them. Free-form turns and command turns share this toolset; a command contributes only its own extras (revision targets, the test-code channel). Re-issuing a command whose turn ended in an error replaces that dead attempt instead of duplicating it. Rewinds and regenerations never destroy content: each removed tail is kept as a branch record (anchored by a prefix fingerprint) and can be switched back to from the fork divider in the pane.
